...
if ( !empty($_POST['userlogin']) && !empty($_POST['passwd']) ) {
        ...
        if ( !$error ) {
                // ========== 수정된 부분 시작 ==========
                // bt_users 테이블에서 직접 user_level과 publisher_id 가져오기
                $user_level = isset($users['user_level']) ? $users['user_level'] : 
                              wps_get_user_meta($users['ID'], 'wps_user_level');
                
                $publisher_id = isset($users['publisher_id']) ? $users['publisher_id'] : null;
                // ========== 수정된 부분 끝 ==========
                ...
                if ( $user_level > 5 ) {
                        ...
                        $_SESSION['login']['publisher_id'] = $publisher_id; // 추가!
                        ...
                        if ( $user_level == 10 ) {
                                ...
                                // 출판사 계정
                                header( 'Location: publisher/book_list.php' );
                        } else {
                                // 일반 관리자
