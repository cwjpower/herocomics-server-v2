...
// 전역 변수 선언
var i, $img, imgWidth, imgHeight, measure, unit, posX, posY, filepath, filename, $activeGridX, $activeGridY;
...
function rulerInit() {
	...
	//console.log($('#pop-local-input').value());
	...
	$('#map-info-wrap').css({ left: imgWidth + $('#workspace-ruler-y').width()+10}); // 정보 영역을 우측으로 append
	...
	if (filename){
		...
		$('#Page').val(page_no); // 페이지 번호
		$('#Width').val(imgWidth-1); // 페이지 width
		$('#Height').val(imgHeight-1); // 페이지 height
	}
...
$(document).on('click', '.gnb-menu-sub li, #gnb-menu li, .form-menu button', menuItemClick); // 메뉴 클릭 함수 호출
...
function generateCode() {
	...
	var initPage = {X:0,Y:0,Width:1015,Height:1560};
	//var initPage =[];
	//initPage['X']=0;
	//initPage['Y']=0;
	//initPage['Width']=1015;
	//initPage['Height']=1560;

	var gcFrames = [initPage]; // 패널 정보, 기본크기 가정
	//var gcFrames = [initPage]; // 패널 정보, 기본크기 가정
	appendPannel(0, initPage);  //
	...
	for (i=0;i<cnt;i++) {
		...
		//Frame['X'] = startCoordX;
		//Frame['Y'] = startCoordY;
		//Frame['Width'] = endCoordX;
		//Frame['Height'] = endCoordY;

		var Frame = {};
		Frame.X = startCoordX;
		Frame.Y = startCoordY;
		Frame.Width = endCoordX - startCoordX;
		Frame.Height = endCoordY - startCoordY;

		//gcFrames.push(Frame);

		console.log("before append");
		appendPannel(i+1, Frame);  //

		//var Frame= '{X:' +  startCoordX + ',' + 'Y:' +  startCoordY + ',' + 'Width:' +  endCoordX + ','+ 'Height:' +  endCoordY + '}' ;
		//gcFrames.push(Frame);
	...
	console.log(gcFrames);
	//$('#Frames').val(JSON.stringify(gcFrames));

	//$('#Frames-hidden').val(JSON.stringify(gcFrames));

}


// 패널 정보 세팅 by endstern
function appendPannel(pannelNo, Frame ){
	var $form_group = $('<div>').attr({class:'form-group'});
	var pannel = 'p['+pannelNo+']';
	var $label = $('<label>').attr({class:'col-sm-2 control-label'}).text(pannel);
	var $div = $('<div>').attr({class:'col-sm-10'});
	console.log(pannel);
	var $input = $('<input type="text">').attr({id:pannel, name:pannel, class:'form-control', val:JSON.stringify(Frame)});

	$input.val(JSON.stringify(Frame));

	$label.appendTo($form_group);
	$input.appendTo($div);
	$div.appendTo($form_group);


	$('#Frames-wrap').append($form_group);
}
